...
class Blockchain(object):
	...
	def new_transaction(self, car_model, chassis_number, mileage):
		"""
...
def new_transaction():
	...
	req = ['car_model', 'chassis_number', 'mileage']
	if not all(x in vals for x in req):
		return 'Missing Required Values!', 400
		
	#Create new transaction
	i = blockchain.new_transaction(vals['car_model'], vals['chassis_number'],
	vals['mileage'])
	
	response = {'message':f'Information added to block: {i}'}
	return jsonify(response), 201
